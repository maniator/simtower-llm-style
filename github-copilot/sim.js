...
class Room {
  constructor(type, floorIndex, startX) {
    this.type = type;
...
class Person {
  constructor(role, origin, target, elevatorType) {
    this.role = role;
    this.origin = origin;
    this.target = target;
...
class ElevatorCar {
  constructor(type, shaftX, capacity) {
    this.type = type;
    this.shaftX = shaftX;
...
export class Game {
  ...
  ensureFloor(index) {
    if (!this.floors.has(index)) {
      this.floors.set(index, {
        index,
        cells: Array(this.width).fill(null),
        rooms: [],
        traffic: 0,
        noise: 0,
        cleanliness: 100,
  ...
  placeRoom(typeId, floorIndex, startX, skipCost = false) {
  ...
  addElevator(type, shaftX) {
    if (type.elevatorType === "stairs") return;
    const car = new ElevatorCar(type.elevatorType, shaftX, type.capacity || 8);
    car.position = 0;
  ...
  spawnTraffic() {
    ...
    if (hour === 10 || hour === 16) {
      const staffElevator = this.elevators.some((car) => car.type === "service")
        ? "service"
        : "standard";
  ...
  chooseElevatorType(origin, target, role) {
    if (
      role === "staff" &&
  ...
  spawnPerson(role, origin, target, elevatorType = null) {
  ...
  findRoomsByCategory(category, idFilter = null) {
    const rooms = [];
    for (const floor of this.floors.values()) {
      for (const room of floor.rooms) {
  ...
  handleElevatorStop(car, floorIndex) {
    const floor = this.getFloor(floorIndex);
  ...
  isExpressStop(floorIndex) {
    return Math.abs(floorIndex) % 5 === 0;
  }

  isElevatorTypeMatch(car, person) {
